...
run-e2e-image:
	...
	oc new-project $(TESTS_NAMESPACE) || oc project $(TESTS_NAMESPACE)
	oc create sa $(TESTS_SA) -n $(TESTS_NAMESPACE) || oc get sa $(TESTS_SA) -n $(TESTS_NAMESPACE)
	oc adm policy add-cluster-role-to-user cluster-admin system:serviceaccount:$(TESTS_NAMESPACE):$(TESTS_SA)
	operator-sdk scorecard ./bundle --namespace=$(TESTS_NAMESPACE) --wait-time=$(TESTS_WAIT_TIME) -o json || oc delete project $(TESTS_NAMESPACE)
...
test-e2e:
	go build -o ./e2e-tests test/main.go
	./e2e-tests TestSelfSignedCerts ./bundle
	./e2e-tests TestACMECertsIngress ./bundle
	./e2e-tests TestCertRenewal ./bundle
.PHONY: test-e2e
